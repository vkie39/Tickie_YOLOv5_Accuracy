...
parser.add_argument('--source', type=str, default="C:/Users/Min/Downloads/y2mate.com - Scooter Freestyle  Worlds Best Pro Scooter Riders_480p.mp4", help='file/dir/URL/glob, 0 for webcam')
...
while cap.isOpened():
    ...
    frame_resized = cv2.resize(frame, (864,480))
    # 객체 감지
    ...
    for *xyxy, conf, cls in pred:
    # 원본 이미지에 대한 좌표로 변환
        xyxy = [int(xyxy[0] * frame.shape[1] / 864),
                int(xyxy[1] * frame.shape[0] / 480),
                int(xyxy[2] * frame.shape[1] / 864),
                int(xyxy[3] * frame.shape[0] / 480)]
